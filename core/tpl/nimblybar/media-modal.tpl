...
<template id="nb_media_insert_img_landscape_tpl">
    <img class="w-full"
      style="max-width: {{width}}px; max-height: {{height}}px;" loading="lazy"
      src="{{src}}" srcset="{{srcset}}" sizes="{{sizes}}" height="{{height}}" width="{{width}}">
</template>
...
<template id="nb_media_insert_doc_tpl">
  <a href="[base_url]/download/{{uuid}}" class="cursor-pointer" download="{{name}}" title="{{description}}">{{title}}</a>
</template>
