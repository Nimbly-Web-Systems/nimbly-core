<form x-data="site_settings([#_ss.name_json#], [#_ss.description_json#], [#_ss.direction_json#], [#_ss.side_json#], [#_ss.language_rows_json#], [#_ss.language_catalog_json#], [#_ss.rtl_json#], [#_ss.features_json#])" @submit.prevent="submit" class="max-w-2xl divide-y divide-neutral-200">
    <div class="pb-5">
        ...
        <ul class="mb-2 flex flex-row" x-show="codes.length > 1" role="group" aria-label="[#text Language of the site name#]"><template x-for="code in codes" :key="code"><li><button type="button" class="cursor-pointer border-b-2 px-4 py-2 text-xs uppercase text-gray-600 hover:font-bold hover:text-black" :class="active.name === code ? 'border-b-primary' : 'border-b-transparent'" :aria-pressed="active.name === code" @click="active.name = code" x-text="code"></button></li></template></ul>
    ...
    <div class="py-5">
        ...
        <ul class="mb-2 flex flex-row" x-show="codes.length > 1" role="group" aria-label="[#text Language of the sidebar position#]"><template x-for="code in codes" :key="code"><li><button type="button" class="cursor-pointer border-b-2 px-4 py-2 text-xs uppercase text-gray-600 hover:font-bold hover:text-black" :class="active.side === code ? 'border-b-primary' : 'border-b-transparent'" :aria-pressed="active.side === code" @click="active.side = code" x-text="code"></button></li></template></ul>
