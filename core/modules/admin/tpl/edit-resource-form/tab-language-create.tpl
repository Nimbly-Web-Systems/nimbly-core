<button type="button" class="flex flex-row uppercase text-xs text-gray-600 px-4 py-2  border-b-2 border-b-transparent
    hover:font-bold hover:text-black cursor-pointer disabled:pointer-events-none disabled:text-gray-400"
    @click='busy=true;translate(`[#item.key#]`)' disabled="true" x-bind:disabled="busy">
    +

    [#text [#item.key#]#]
    
    <svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" fill="none" viewBox="0 0 128 128" 
        class="w-5 h-5 mx-1 -mt-0.5 animate-spin" 
        x-cloak
        x-show="busy">
        <path stroke="#1B1B1B" stroke-width="6"
