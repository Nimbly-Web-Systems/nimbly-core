<button type="button" class="absolute right-[2px] top-[2px] h-[24px] w-[24px] text-neutral-600 
    hover:bg-neutral-200 hover:rounded-full hover:text-primary"
        x-show="![#_fmodel#]"
        x-data="{me_busy: false}"
        :disabled="me_busy"
        :class="me_busy && 'disabled'"
       @click="me_busy=true; ai(`[#item.key#]`, lang)" >
    <svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" fill="none" viewBox="0 0 128 128" 
        class="w-6 h-6"
        :class="me_busy && 'animate-spin'">
