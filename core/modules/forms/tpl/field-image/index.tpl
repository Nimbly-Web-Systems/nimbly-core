<div class="relative my-6 border border-neutral-300 rounded bg-neutral-50 p-4" data-nb-edit-image="[#_f.key#]"
    ...
    <button class="[#btn-class-icon#] absolute right-12 top-1 p-1 text-neutral-600" data-te-toggle="modal"
        data-te-target="#nb-modal-insert-media" @click.prevent="select_image('[#_f.key#]')">
    ...
    <button class="[#btn-class-icon#] absolute right-3 top-1 p-1 text-neutral-600
        disabled:pointer-events-none disabled:text-neutral-200"
        :disabled="![#_f.model#]" @click.prevent="delete_image('[#_f.key#]')">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
            class="w-4 h-4">
            <path stroke-linecap="round" stroke-linejoin="round"
