<script>var _nb_multidate_[#_f.key#] = [#fmt val=_f.value json empty=[]#];</script>
<div class="relative my-10 border border-neutral-300 rounded bg-neutral-50 p-4"
    x-data="{ new_date: '' }"
    x-init="form_data['[#_f.key#]'] = window._nb_multidate_[#_f.key#] || []; delete window._nb_multidate_[#_f.key#]">
    ...
    <div class="flex flex-col gap-1 mt-1">
        <template x-for="d in form_data['[#_f.key#]']" :key="d">
            <div class="flex items-center justify-between bg-white border border-neutral-200 rounded px-3 py-1.5">
                ...
                <button type="button"
                    @click="form_data['[#_f.key#]'] = form_data['[#_f.key#]'].filter(x => x !== d)"
