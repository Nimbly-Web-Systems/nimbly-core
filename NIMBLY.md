...
- **Manage** — grouped clusters, one per resource, laid out with equal height (`items-stretch` on the row) so the row reads as one set, not cards of mismatched size. Each cluster follows a strict internal order: a dark small uppercase heading, a caption fact (same `text-neutral-500` as Site status, for a consistent look), then — only if the cluster has one — its single genuinely primary/frequent action (e.g. `Add user`, using the project's real primary-button style, `core/modules/admin/lib/dashboard/quick-action-primary-link.tpl`, matching the Site Settings save button), then a final **secondary row** holding everything else together: the entry pill(s) (e.g. `Users (3)`) *and* any remaining actions, all at the same plain/calm visual weight. Putting entry pills in the secondary row rather than making them a second visual anchor is deliberate — a cluster should have at most one strong thing to look at, not the primary button competing with its own navigation pill:
  - **Users & roles**: "N active sessions" caption, **Add user** (primary), then `Users (N)` / `Roles (N)` entry pills / `Clear all sessions` together in the secondary row.
  - **Media library**: "Updated X ago" caption (the library's own last-modified time, same shape as the other captions), no primary action, then `Media Library (N)` / `Clear media cache` / `Delete unused media` in the secondary row.
  - **Jobs**: "Scheduler last ran" caption, no primary action, then `Jobs (N)` (N = currently queued jobs) / `Run due jobs now` in the secondary row (posts to `/nb-admin/jobs`'s own handler, not the dashboard's).

  "Add role" is deliberately not a cluster action — not a daily action, so Roles is a plain entry pill, not a create button. The Jobs cluster doubles as the only entry point into `/nb-admin/jobs` anywhere in the admin. Site settings is **not** a Manage cluster — it has no fact of its own to show, so it's a small gear-icon button in the dashboard's own header (top-right, `core/modules/admin/uri/nb-admin/dashboard-settings-btn.tpl`, gated on `edit-.config`) instead.

  Needs attention's items follow the same "fact, then a light secondary pill action" shape (`core/modules/admin/lib/dashboard/attention.tpl`) — no em dashes separating message from action; the layout (`flex justify-between`) does that job instead.
