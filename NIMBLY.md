...
- Don't repeat long Tailwind class lists across links, buttons, or related UI elements. Extract the canonical class list into a small `.tpl` class partial (for example `cls-action-link.tpl`) and reuse its rendered value. Place it at the narrowest useful scope: beside the route in `core/modules/<module>/uri/<route>/` or `ext/uri/<route>/` for page-scoped styles, in `core/tpl/` for framework-wide styles, or in `ext/tpl/` for reusable application-specific styles. Keep one-off classes inline; extract them when the same visual contract starts repeating.
