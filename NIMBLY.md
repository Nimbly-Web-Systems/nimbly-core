...
- Don't repeat substantial Tailwind class lists across links, buttons, or related UI elements. When the same long visual contract repeats, extract its canonical class list into a small `.tpl` class partial (for example `cls-action-link.tpl`) and reuse its rendered value. Place it at the narrowest useful scope: beside the route in `core/modules/<module>/uri/<route>/` or `ext/uri/<route>/` for page-scoped styles, in `core/tpl/` for framework-wide styles, or in `ext/tpl/` for reusable application-specific styles. Apply this only when the abstraction provides a real consistency or maintenance benefit. Keep one-off classes, single utilities, and short class lists inline; extracting those adds indirection without enough value.
...
Nimbly projects, do not create one cron entry per project. Use the server-level
scheduler orchestrator instead and let it call each registered project's
`schedule:run` sequentially.
...
Core records every request that reaches `index.php` without cookies or
JavaScript. Recording is on for `APP_ENV=prod` and `stage`; set
`STATS_ENABLED=1` or `0` to override. Files served directly by `.htaccess`
(`ext/static`, `core/static`, `/img/` thumbnails) and blocked `.php` probes
...
`stats:import-apache` reads Apache access log lines (`combined` or
`vhost_combined`) from stdin, so only the log read needs root:

```bash
zcat -f /var/log/apache2/site-access.log.* | sudo -u www-data ./nimbly stats:import-apache --mode=full --before=2026-09-25T02:02:00Z
zcat -f /var/log/apache2/site-access.log.1 | sudo -u www-data ./nimbly stats:import-apache
```

`--mode=full` backfills every line before `--before` (days before recording
started). The default `--mode=enrich` only adds what never reaches PHP: files
served from `ext/static` or `core/static`, `/img/` thumbnails and blocked
`.php` probes. Lines are written to `apache-YYYY-MM-DD.log` (UTC dates) and
archived by the next rollup; a watermark in `.tmp/stats/apache.watermark` skips
lines already imported. Use `--base=/subdir/` for subdirectory installs and
`--host=` when `SITE_URL` is not set. Apache lines have no `Accept-Language`,
content type or login state: content types are guessed from the path and
editors count as visitors.

`STATS_KEY` (64 hex characters, generated by `system:setup`) encrypts the raw
archive. Back it up: without it the raw history cannot be read. While it is
missing, rollup fails and keeps the running logs. `stats:raw YYYY-MM-DD
[--apache]` prints a decrypted day.
...
The orchestrator uses these server-managed files by default:

| Path | Purpose |
|---|---|
| `/usr/local/bin/nimbly-scheduler-orchestrator` | Wrapper run by cron |
| `/etc/nimbly/scheduler-projects.json` | Registered projects and defaults |
| `/etc/cron.d/nimbly-scheduler` | System cron entry |
| `/var/log/nimbly-scheduler.log` | Scheduler output log |
...
- **Manage** — a section card like the others (`rounded-2xl bg-neutral-50 p-6 shadow`, same heading style as Your data), containing grouped clusters, one per resource, laid out with equal height (`items-stretch` on the row) so the row reads as one set rather than cards of mismatched size. Each cluster follows a strict internal order: a dark small uppercase heading, a caption fact (same `text-neutral-500` as Site status), then — only if the cluster genuinely has one — its single most important pill (e.g. `Media Library (N)`, `Jobs (N)`, or `Add user` when a cluster's main draw is a create action rather than a list), and finally a **secondary row** where everything else (remaining entry pills like `Users (N)`/`Roles (N)`, plus every other action) renders as plain underlined text links, all sharing one canonical link style (`dashboard_secondary_link_class()` in `dashboard.php`, reused as-is by the Site status "Update now"/"View debug" links and every `quick-action-*.tpl` partial — one style everywhere, not a per-context variant). A cluster has exactly one pill-weight element, never two competing for attention:
  ...
  "Add role" is deliberately not a cluster action — not a daily action, so Roles is a plain secondary link, not a create button. The System cluster doubles as the only entry point into `/nb-admin/jobs` anywhere in the admin. Site settings is **not** a Manage cluster — it has no fact of its own to show, so it's a small gear-icon button in the dashboard's own header (top-right, `core/modules/admin/uri/nb-admin/dashboard-settings-btn.tpl`, gated on `edit-.config`) instead.

  Needs attention's items follow a similar "fact, then a light secondary pill action" shape (`core/modules/admin/lib/dashboard/attention.tpl`) — no em dashes separating message from action; the action sits on its own line below the message instead.

  Every Site status item and Manage cluster heading (`dashboard_status_heading()` in `dashboard.php`) uses the project's actual brand color (`text-primary`, from `ext/tailwind.theme.js`) rather than plain gray — one consistent accent, not a different hue per category. A per-category rainbow was tried and rejected (read as visual noise); a small colored icon badge per heading was also tried and rejected (it indented the label relative to the caption/value below it, breaking the column's flush-left edge). `text-primary` stays reserved for these headings only — amber/red keep their separate meaning (attention/warning) elsewhere on the page.
...
`/nb-admin/jobs` (`core/modules/admin/lib/jobs-panel/`) is a diagnostic view over the job queue and scheduler — queued/running/done/failed counts, a table of jobs with their last error, and "Run due jobs now" / "Prune old jobs" actions, gated on `view-.jobs` / `manage-.jobs`. "Prune old jobs" removes both `done` jobs (aged off `completed_at`) and terminally `failed` jobs with no retries left (aged off `_modified`) older than 30 days — the same rule the scheduled `jobs:prune --days=30` task applies, so a fixed-and-forgotten failure eventually drops off the **Needs attention** band on its own. Each `done`/`failed` row also has its own **Delete** button (gated on `manage-.jobs`) for clearing an individual job immediately regardless of age — queued/running rows show it disabled, since deleting an in-flight job could orphan a lock or a running process.

`/nb-admin/settings` (`core/modules/admin/lib/site-settings/`) is one flat page gated on `edit-.config`. It edits the site-wide `.config/site` record: name, description, languages, text direction (`direction`, `ltr`/`rtl`, rendered as `<html dir>`) and admin sidebar position (`nimblybar.side`). Name, description, direction and side are per-language values, and each field has its own language switch. On save, direction and side are filled in for every site language: Arabic, Persian and Hebrew start `rtl` with the sidebar on the right, and other languages start `ltr` and left. For users with `manage-system`, it also holds the custom pages switch with its page types, and the navigation editing switch (`.config/managed_pages`). Each switch shows only when there is a `pages` resource or navigation slots to switch on. The Pages and Navigation admin tabs are hidden for everyone while their switch is off. This is distinct from the per-page title editor behind the nimblybar gear icon, which edits `page_settings.page_title` for the current page only.
