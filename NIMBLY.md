...
- **Site status** — is the site current and healthy, in one glance. Each item leads with whichever fact actually answers "is this OK": for **Data**, that's *when* (a large "17 hours ago", naming the specific resource below it in small text — e.g. "Projects updated" — so it's directly checkable against the matching row in Your data, not an anonymous number); for **Core**/**Ext**, that's *status* (a large "Up to date" or "N updates", the number in amber when something's pending), with the raw "Updated X ago" timestamp demoted to a small caption underneath, plus an inline **Update now** for roles with `pull-core-updates`/`pull-ext-updates`; for **System** (`view-debug`), there's nothing urgent enough to warrant large text, so it stays a small RAM/disk-free fact with a **View debug** link into `/nb-admin/debug`. Everything here is a fact (with an action attached where one applies), never bare navigation — that's what Manage below is for.
- **Your data** — the resources the current role can see, with record counts, disk usage, and last-updated time per resource.
- **Manage** — grouped clusters, one per resource, each with a small uppercase heading, calm outline-pill entry link(s) with a record count (e.g. `Users (3)`), and any actions rendered as plain underlined text links rather than another row of pill buttons — so the eye has exactly one visual weight per cluster to land on, not several equally-loud buttons competing:
  - **Users & roles**: `Users (N)` and `Roles (N)` entry pills — they're managed together — plus `Add user` / `Clear all sessions` text-link actions, "N active sessions" caption.
  - **Media library**: `Media Library (N)` entry pill, `Clear media cache` / `Delete unused media` text-link actions.
  - **Jobs**: `Jobs (N)` entry pill (N = currently queued jobs), `Run due jobs now` text-link action (posts to `/nb-admin/jobs`'s own handler, not the dashboard's), "Scheduler last ran" caption.
