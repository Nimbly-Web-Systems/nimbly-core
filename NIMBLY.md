...
- **Manage** — a section card like the others (`rounded-2xl bg-neutral-50 p-6 shadow`, same heading style as Your data), containing grouped clusters, one per resource, laid out with equal height (`items-stretch` on the row) so the row reads as one set rather than cards of mismatched size. Each cluster follows a strict internal order: a dark small uppercase heading, a caption fact (same `text-neutral-500` as Site status), then — only if the cluster genuinely has one — its single most important pill (e.g. `Media Library (N)`, `Jobs (N)`, or `Add user` when a cluster's main draw is a create action rather than a list), and finally a **secondary row** where everything else (remaining entry pills like `Users (N)`/`Roles (N)`, plus every other action) renders as plain underlined text links, all sharing one canonical link style (`dashboard_secondary_link_class()` in `dashboard.php`, reused as-is by the Site status "Update now"/"View debug" links and every `quick-action-*.tpl` partial — one style everywhere, not a per-context variant). A cluster has exactly one pill-weight element, never two competing for attention:
  - **Users & roles**: "N active sessions" caption, `Add user` pill (the create action is the main reason to visit this cluster), then `Users (N)` / `Roles (N)` / `Clear all sessions` as secondary text links.
  - **Media library**: "Updated X ago" caption (the library's own last-modified time), `Media Library (N)` pill, then `Clear thumbnail cache` / `Clear data cache` / `Delete unused media` as secondary text links. The two caches are distinct: thumbnail cache is `ext/static/_thumb_` (image resizes); data cache is `ext/data/.tmp/cache/_data`, the per-resource `data_read_all()` result cache from `core/lib/data.php` — normally self-invalidating on resource modification, but clearable here for troubleshooting.
  - **Jobs**: "Scheduler last ran" caption, `Jobs (N)` pill (N = currently queued jobs), then `Run due jobs now` as a secondary text link (posts to `/nb-admin/jobs`'s own handler, not the dashboard's).

  "Add role" is deliberately not a cluster action — not a daily action, so Roles is a plain secondary link, not a create button. The Jobs cluster doubles as the only entry point into `/nb-admin/jobs` anywhere in the admin. Site settings is **not** a Manage cluster — it has no fact of its own to show, so it's a small gear-icon button in the dashboard's own header (top-right, `core/modules/admin/uri/nb-admin/dashboard-settings-btn.tpl`, gated on `edit-.config`) instead.
  ...
  Every Site status item and Manage cluster heading carries a small icon in a soft colored circle (`dashboard_badge()`/`dashboard_icon()` in `dashboard.php`) — a categorical color per concept (Data blue, Core indigo, Ext teal, System slate, Users & roles sky, Media library violet, Jobs emerald), deliberately distinct from the amber/red reserved for actual warning states elsewhere on the page, so adding icons doesn't dilute the "amber means look at this" signal. Icon paths are plain Heroicons-outline `d=` strings, reusing exact paths already used elsewhere in the admin where one existed (e.g. the resources stack icon from `core/tpl/nimblybar/menu-resources.tpl` for Data) rather than inventing a parallel icon language.
