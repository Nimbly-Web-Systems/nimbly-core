...
- **Manage** — grouped clusters, one per resource, laid out with equal height (`items-stretch` on the row) so the row reads as one set, not cards of mismatched size. Each cluster follows the same internal order: a dark small uppercase heading, calm outline-pill entry link(s) with a record count (e.g. `Users (3)`), a caption fact (same `text-neutral-500` as Site status, for a consistent look), *then* the actions — captions sit above the actions, not below, so the fact reads before the affordance. Actions are plain underlined text links **except** the one genuinely primary, frequent action per cluster (e.g. `Add user`), which uses the project's real primary-button style (`core/modules/admin/lib/dashboard/quick-action-primary-link.tpl`, matching the same class the Site Settings save button already uses) — so the eye has exactly one strong visual anchor per cluster, not several equally-loud buttons competing:
  - **Users & roles**: `Users (N)` and `Roles (N)` entry pills — they're managed together — "N active sessions" caption, then **Add user** (primary) / `Clear all sessions` (text link) actions.
  - **Media library**: `Media Library (N)` entry pill, "Updated X ago" caption (the library's own last-modified time, same shape as the other captions), then `Clear media cache` / `Delete unused media` text-link actions.
  - **Jobs**: `Jobs (N)` entry pill (N = currently queued jobs), "Scheduler last ran" caption, then `Run due jobs now` text-link action (posts to `/nb-admin/jobs`'s own handler, not the dashboard's).
